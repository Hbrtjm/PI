...
{
    ht_element *current, *tmp;
    for(int i = 0;i < p_table->size;i++)
    {
        current = p_table->ht[i].next;
        while(current != NULL)
        {
            tmp = current;
            current = current->next;
...
void rehash(hash_table *p_table)
{
    ...
    ht_element *to_insert,*current,*new_table = calloc(new_size, sizeof(ht_element));

    for(int i = 0; i < p_table->size; i++){
        current = p_table->ht[i].next;
        
        while(current != NULL){
            size_t index = p_table->hash_function(current->data, new_size);
            to_insert = malloc(sizeof(ht_element));
            to_insert->data = current->data;
            to_insert->next = new_table[index].next;
            new_table[index].next = to_insert;
            ht_element* temp = current;
            current = current->next;
            free(temp);
        }
    }
...
ht_element *get_element(hash_table *p_table, data_union *data) {
    ...
    while(current != NULL && p_table->compare_data(current->data, *data) != 0)
        current = current->next;
...
{
    ht_element *to_remove = get_element(p_table,&data),*previous = find_previous(p_table,data);
    if(to_remove == NULL)
    {
        return;
    }
    previous->next = to_remove->next;
    free_element(p_table->free_data,to_remove);
}
