...
int read_int_lines(line_type lines_array[]) 
{
	...
	{
		line_size = 0;
		line_array = read_int_lines(&line_size);
		if(line_array == NULL)
		{
			return lines_count;
		}
		line_array[lines_count].values = malloc(sizeof(int)*line_size);
		line_array[lines_count].len = line_size;
...
{
	qsort(line_array,line_count,sizeof(line_type),cmp);
}
...
void make_CSR(triplet *triplet_array, int n_triplets, int rows, int *V, int *C, int *R) 
{
	...
	for(int i = 0;i < n_triplets;i++)
	{
		...
		for(int j = triplet_array.r + 1;j < rows + 1;k++)
		{
			R[j] += 1;
		}
	}
